#!/usr/bin/env python3
"""Create fail-closed evidence for the Stage 6R-9 native 84-test gate."""
    ...
    *,
    ...
    checks = {
        ...
        **{
        ...
        "nativeTotal84Of84": sum(suite.expected for suite in SUITES) == 84
...
def build_evidence(args: argparse.Namespace, log_bytes: bytes) -> dict[str, object]:
    ...
    return {
        "schemaVersion": "1.0",
        "stage": "6R-9",
        "gate": "oidc-tenant-self-approval-full-regression",
...
def write_summary(path: Path, evidence: dict[str, object]) -> None:
    ...
    lines = [
        "# Stage 6R-9 Mobile ETag and role DTO full regression",
        "",
    ...
    lines.extend(
        [
            f"| Native合計 | 84/84={checks['nativeTotal84Of84']} |",
