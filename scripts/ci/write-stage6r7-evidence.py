#!/usr/bin/env python3
"""Create fail-closed evidence for the Stage 6R-7 native 81-test gate."""
    ...
    *,
    ...
    checks = {
        ...
        **{
        ...
        "nativeTotal81Of81": sum(suite.expected for suite in SUITES) == 81
...
def build_evidence(args: argparse.Namespace, log_bytes: bytes) -> dict[str, object]:
    ...
    return {
        "schemaVersion": "1.0",
        "stage": "6R-7",
        "gate": "append-only-full-regression",
...
def write_summary(path: Path, evidence: dict[str, object]) -> None:
    ...
    lines = [
        "# Stage 6R-7 Append-only database full regression",
        "",
    ...
    lines.extend(
        [
            f"| Native合計 | 81/81={checks['nativeTotal81Of81']} |",
