#!/usr/bin/env python3
"""Create fail-closed evidence for the Stage 6R-8 native 83-test gate."""
    ...
    *,
    ...
    checks = {
        ...
        **{
        ...
        "nativeTotal83Of83": sum(suite.expected for suite in SUITES) == 83
...
def build_evidence(args: argparse.Namespace, log_bytes: bytes) -> dict[str, object]:
    ...
    return {
        "schemaVersion": "1.0",
        "stage": "6R-8",
        "gate": "mobile-etag-role-dto-full-regression",
...
def write_summary(path: Path, evidence: dict[str, object]) -> None:
    ...
    lines = [
        "# Stage 6R-8 Mobile ETag and role DTO full regression",
        "",
    ...
    lines.extend(
        [
            f"| Native合計 | 83/83={checks['nativeTotal83Of83']} |",
