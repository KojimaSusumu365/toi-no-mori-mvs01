...
| Run | 結果 | 判定 |
|---:|---|---|
| #1 | 84/85、TC-078だけRED | 期待どおりの失敗先行 |
| #2 | DR artifactはaccepted、`false`値のテスト式が誤判定 | テスト不具合として不採用 |
| #3 | 85/85だが、TC-030の外部コマンド依存をログ再監査で検出 | 暗号文検査の移植性を補強するまで保留 |
| #4 | 非root native 85/85、コマンド欠落なし | 正式受入 |

最終受入のrun、hash、測定値は`stage6r10-github-acceptance.md`を正本とする。
