...
| REQ-MVS01-AUD-001 | 許可リスト型の追記監査 | API TC-023、PostgreSQL TC-025 | DB同一トランザクションまで実装済み |
...
| REQ-MVS01-UI-002 | Reviewer queue、公開中一覧、差戻し、承認、取り下げ、監査 | API TC-050/054、Mobile TC-055 | role別画面と状態絞込を実装済み |
...
| ADR-0007-D1/D3 | tenant列、transaction-local `set_config`、RLS ENABLE/FORCE、非owner applicationロール | API TC-066-API、PostgreSQL TC-066/067-PG | 2接続・異role・双方VerifyFull・最小GRANT・Production起動診断はAPI 36/36でGREEN。native PGはbuild済み、実DB未実行 |
...
| QF-ST6R4C-MVS01-001 | Ubuntu 24.04非root CI、必須DB gate、機械判定証跡 | CI構成契約6件＋PostgreSQL native 10件 | 構成契約6/6 GREEN。GitHub native 10件は未実行 |
...
| UML-UC/NAV-MVS01-006 | Editor/Reviewer/Publicとrole別画面遷移 | API TC-049/050、Mobile TC-055 |
...
Pull Requestの完了条件は、要求ID、UML ID、実装、Domain/API/Mobile/OIDC E2E/PostgreSQL/DRテストIDのリンクが同じ変更内で維持されることです。Stage 6R-4の今回実行範囲はDomain/API/Mobile/OIDC E2Eの61件で、60件合格・TC-055期待RED 1件です。PostgreSQL 10件/DR 4件、Stage 6R残存11件、実IdP、実browser/スマートフォン、さくら実クラウドの判定は結果を混同せず別gateとします。
