...
CI構成契約は6/6合格し、このWork環境ではroot時にnative suiteを開始せずexit 2となる失敗閉鎖を確認した。実際のGitHub Actions runはまだ行っていないため、PostgreSQL 10/10は未合格のままである。詳細は`docs/stage6r4c-nonroot-postgresql-ci.md`を参照する。
